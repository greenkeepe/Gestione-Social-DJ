...
export async function POST(req: Request) {
  ...
  if (!accountId || !accessKeyId || !secretAccessKey || !bucketName || !dashboardPublicUrl) {
    return NextResponse.json(
      { error: "Configurazione R2 mancante sul server (R2_ACCOUNT_ID/R2_ACCESS_KEY_ID/R2_SECRET_ACCESS_KEY/R2_BUCKET_NAME/DASHBOARD_PUBLIC_URL)." },
      { status: 500 }
    );
  }
  ...
  const publicUrl = `${dashboardPublicUrl.replace(/\/$/, "")}/api/r2-file/${chiaveOggetto}`;
